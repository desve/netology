...
# Извлекаем файлы и обрабатываем их
        file_path_output = os.path.join(PATH, 'Result', file)
        command = 'convert {} -resize 200 {}'.format(file, file_path_output)
        go = subprocess.Popen(command, shell = True)
...
def files_convert_4(files):
    ...
    for file in files:
        file_path_output = os.path.join(PATH, 'Result', file)
        command = 'convert {0} -resize 200 {1}'.format(file, file_path_output)
        
#        go = subprocess.call(command, shell = True)    # с этой командой работает   
